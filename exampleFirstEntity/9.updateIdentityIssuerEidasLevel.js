...
async function main() {
  unlockAccount()
  console.log('\n ------ Example of updating Issuer eIDAS level ------ \n')
  const transactionUpdateEidasLevel = await transactionFactory.identityManager.updateIdentityIssuerEidasLevel(
    web3,
    configData.didEntity1,
    configData.issuerLevel + 1
  )
  const getKnownTxAddIssuer = await adminIdentity.getKnownTransaction(
    transactionUpdateEidasLevel
  )
